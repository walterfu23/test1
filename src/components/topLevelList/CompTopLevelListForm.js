...
// The form in a dialog to add or edit fields for TopLevelList
...
// 
class CompTopLevelListForm extends Component {
  ...
  constructor(props) {
    ...
    const dialogTitle = props.creating ? 'Add TopLevelList' : 'Edit TopLevelList';
    const stateInit = {
          ...
          >
            ...
            <div className="drp-margin-bottom">
              <Input
                ...
                label="List Label"
                required={true}
                value={this.state.recInEdit.ListLabel || ''}
                onChange={this.onDialogInputChange}
                style={{ width: "100%" }}
              />
            </div>
            <div className="drp-margin-bottom">
              <Input
                ...
                label="List Name"
                required={true}
                value={this.state.recInEdit.ListName || ''}
                onChange={this.onDialogInputChange}
                style={{ width: "100%" }}
              />
            </div>
            ...
            <br />
            <ErrorBox loc="TopLevelList_form" />
            <br />
            <div>
              <button
...
const mapDispatchToProps = (dispatch, props) => {
  return {
    createRequested: (rec) => dispatch(
      actionGen(actionTopLevelList.CREATE_TopLevelList_REQUESTED, rec)
    ),
    updateRequested: (rec) => dispatch(
      actionGen(actionTopLevelList.UPDATE_TopLevelList_REQUESTED, rec)
    ),
  }
}

export default connect(mapStateToProps, mapDispatchToProps)(CompTopLevelListForm);
