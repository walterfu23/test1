...
import CompJobTopLevelListForm from './CompJobTopLevelListForm';
...
class CompJobTopLevelList extends Component {
  ...
  //==========================================================
  // render for CompJobTopLevelList
  render() {
    const hasCurrentRec = !utils.objEmpty(this.props.getCurrentRec);
    return (
      <div>
        <ErrorBox loc="JobTopLevelList_main" />
        ...
        >
          ...
          <GridColumn field="Job.Label" title="Job" width="200px" />
          <GridColumn field="TopLevelList.ListLabel" title="List"  />
...
const mapStateToProps = (state, props) => {
  const listRecsUnsorted = createJobTopLevelListListSelector(state);
  const listRecs = JobTopLevelList.sortByIdDesc(listRecsUnsorted);
  return {
    listRecs,       // list of JobTopLevelList's
    getShowLoading: actionControl.getShowLoadingJobTopLevelList(state),
    getShowForm: actionControl.getShowFormJobTopLevelList(state),
...
const mapDispatchToProps = (dispatch, props) => {
  return {
    setShowForm: (showFlag) => dispatch(
      actionControl.setShowFormJobTopLevelList(showFlag)),
    deleteRequested: (rec) => dispatch(
      actionGen(actionJobTopLevelList.DELETE_JobTopLevelList_REQUESTED, rec)
    ),
    setCurrentRec: (rec) => dispatch(
      actionControl.setCurrentJobTopLevelList(rec)
    ),
  }
}

const Comp_confirm = withCompConfirmDialog(CompJobTopLevelList);
